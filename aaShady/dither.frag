...
float sdGyroid( vec3 p, float w)
{
    float v = sin(p.x)*cos(p.y) + sin(p.y)*cos(p.z) + sin(p.z)*cos(p.x);
    return abs(v) - 0.3;
}

float sdDiamond( vec3 p, float w)
{
    float v = cos(p.x)*cos(p.y)*cos(p.z) - sin(p.x)*sin(p.y)*sin(p.z);
    return abs(v) - 0.2;
}
...
float Scene(vec3 pos)
{
    ...
    float gyr = opI(sdSphere(pos, 2.*PI), sdDiamond(pos, 1.0));

...
let epsSigma = 3.0;        // base Gaussian sigma in grid cells for Sinkhorn kernel
let iters = 80;            // Sinkhorn iterations
...
function renderDensity(rho) {
  ...
  const sal = { r: 0xFA, g: 0x80, b: 0x72 };     // salmon
  ...
  let arr = rho;
  let maxVal = percentile(arr, 99.5); // robust cap
  ...
  for (let j = 0; j < NY; j++) {
    for (let i = 0; i < NX; i++) {
      ...
      const r = sky.r + v * (sal.r - sky.r);
      const g = sky.g + v * (sal.g - sky.g);
      const b = sky.b + v * (sal.b - sky.b);
      fill(r, g, b);

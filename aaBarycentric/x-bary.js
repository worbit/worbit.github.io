...
function setup() {
  ...
  //cons = createElement('p','hallo').style('font-size', '16pt');
  textSize(20);
...
function draw() {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      // if (inTriangle(P, Vs[0], Vs[1], Vs[2])
      //  || inTriangle(P, Vs[0], Vs[2], Vs[3])) {
        let r = min(a1[0], a2[0]);
        let g = min(a1[1], a2[1]);
        let b = min(a1[2], a2[2]);
        r = a1[2];
        ...
        set(x, y, mycol);
        //set(x, y, color('salmon'));
      } else {
        //set(x, y, color('skyBlue'));
        let r = min(a1[0], a2[0]);
        let g = min(a1[1], a2[1]);
        let b = min(a1[2], a2[2]);
        r = a1[2];
  ...
  updatePixels();
  //cons.html(`mn: ${mn.toFixed(2)} mx: ${mx.toFixed(2)}`);
  if (checkbox.checked()) {
    let mox = mouseX;
    let moy = mouseY;
